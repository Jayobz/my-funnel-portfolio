...
function isLogo(project: DesignProject): boolean {
  return "projectType" in project && (project as { projectType?: string }).projectType === "logo";
}

function getClient(project: DesignProject): string | null {
  return "client" in project ? (project as { client?: string }).client ?? null : null;
}

function getServiceType(project: DesignProject): string | null {
  return "serviceType" in project
    ? (project as { serviceType?: string }).serviceType ?? null
    : null;
}

function getDeliverable(project: DesignProject): string | null {
  return "deliverable" in project
    ? (project as { deliverable?: string }).deliverable ?? null
    : null;
}
  ...
  if (project.imagePath) {
    return (
      ...
      <img
        ...
        alt={
          isLogo(project)
            ? `${project.title} logo designed for ${getClient(project) ?? "client"}`
            : project.title
        }
        onClick={onClick}
        className={`w-full ${fit === "contain" ? "object-contain" : isLogo(project) ? "object-contain" : "object-cover"} ${onClick ? "cursor-zoom-in" : ""} ${className}`}
      />
    );
  }

  // Styled placeholder (poster only)
  return (
    <div
      onClick={onClick}
    ...
    >
      ...
      <p className="relative z-10 mt-3 text-[11px] font-medium italic text-amber-200/70">Romans 12:2</p>
    ...
    >
      <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={zoomOut} disabled={zoom <= MIN} aria-label="Zoom out"
          ...
          <button onClick={reset} aria-label="Reset zoom"
            ...
            <RotateCcw className="mr-1.5 h-3 w-3" />{Math.round(zoom * 100)}%
          </button>
          <button onClick={zoomIn} disabled={zoom >= MAX} aria-label="Zoom in"
            className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/5 text-white/70 transition hover:bg-white/10 hover:text-white disabled:opacity-30">
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4 pl-8">
        {steps.map((step, i) => (
          <motion.div key={step} variants={staggerItem} className="relative flex items-center gap-3">
...
function SectionHeading({ number, title }: { number: string; title: string }) {
  return (
    <div className="mb-6 flex items-baseline gap-3">
      <span className="text-xs font-bold tracking-[0.2em] text-orange-400/70">{number}</span>
...
// ─── Logo Hero ────────────────────────────────────────────────────────────────
// Premium presentation for logo/branding projects
...
function LogoHero({
  ...
  const deliverable = getDeliverable(project);

  return (
    <div className="grid lg:grid-cols-[1.1fr_1fr]">
      {/* Logo showcase — clean white/neutral bg */}
      <motion.div
        initial={{ opacity: 0, x: -20 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ delay: 0.15, duration: 0.5 }}
        className="relative flex min-h-[320px] items-center justify-center overflow-hidden bg-gradient-to-br from-[#f8f6f2] to-[#ede9e0] lg:min-h-[440px]"
      >
        {/* Subtle grid */}
        <div
          aria-hidden="true"
        ...
        >
          {project.imagePath && (
            ...
            <img
              src={project.imagePath}
              alt={`Bulaklaya Mjass logo designed for ${client ?? "client"}`}
              className="max-h-[260px] w-auto cursor-zoom-in object-contain drop-shadow-[0_8px_32px_rgba(0,0,0,0.18)]"
            />
          )}
        </motion.div>
        {/* Zoom hint */}
        <button
          ...
          className="absolute bottom-3 right-3 flex items-center gap-1.5 rounded-full bg-black/12 px-3 py-1.5 text-[11px] font-medium text-black/50 backdrop-blur-sm transition hover:bg-black/20"
        >
      ...
      >
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 text-sm">
          {[
            ...
            { label: "Role",        value: project.role.title },
...
export function DesignProjectModal({
  ...
  const handleKey = useCallback(
    (e: KeyboardEvent) => { if (e.key === "Escape" && !viewerOpen) onClose(); },
    [onClose, viewerOpen]
  );
  useEffect(() => {
    ...
    <>
      ...
      >
        ...
        <button
          ...
          className="absolute right-4 top-4 z-20 flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-white/5 text-slate-400 transition hover:bg-white/10 hover:text-white focus:outline-none focus:ring-2 focus:ring-orange-400"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto overscroll-contain">
          ...
          {/* Hero — switches based on project type */}
          {logo ? (
            <LogoHero project={project} onZoom={() => setViewerOpen(true)} />
          ) : (
            <PosterHero project={project} onZoom={() => setViewerOpen(true)} />
          )}
          ...
          <div className="space-y-14 px-6 py-12 md:px-10">
            ...
            {/* 01 — Overview */}
            <motion.div variants={revealLightVariants} initial="hidden" whileInView="visible" viewport={viewport}>
              <SectionHeading number="01 —" title="Overview" />
              <p className="max-w-2xl text-base leading-8 text-slate-300">{project.overview}</p>

              {/* Logo: show project details grid; Poster: show event info grid */}
              {logo ? (
                <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4 text-sm">
                  {[
                    { label: "Client",      value: getClient(project) ?? "—"      },
                    { label: "Project Type",value: getServiceType(project) ?? "—" },
                    { label: "Category",    value: project.category               },
                    { label: "Deliverable", value: getDeliverable(project) ?? "—" },
                  ].map(({ label, value }) => (
                    <div key={label} className="rounded-xl border border-white/8 bg-white/[0.03] px-3 py-3">
                      <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500">{label}</p>
                      <p className="mt-1 text-xs font-medium text-white break-words">{value}</p>
                    </div>
                  ))}
                </div>
              ) : (
            ...
            <motion.div variants={revealLightVariants} initial="hidden" whileInView="visible" viewport={viewport}>
              ...
              <motion.ul variants={staggerContainer} initial="hidden" whileInView="visible" viewport={viewport} className="space-y-3">
                {project.objective.map((item) => (
                  <motion.li key={item} variants={staggerItem} className="flex items-start gap-3 text-sm leading-7 text-slate-300">
            ...
            {/* 03 — Design Approach / Concept */}
            <motion.div variants={revealLightVariants} initial="hidden" whileInView="visible" viewport={viewport}>
              <SectionHeading number="03 —" title={logo ? "Design Approach" : "Design Concept"} />
              <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={viewport} className="grid gap-4 sm:grid-cols-2">
                {project.designConcept.map(({ element, meaning }) => (
                  ...
                  >
                    <p className="mb-1.5 text-sm font-semibold text-orange-300">{element}</p>
            ...
            {/* 04 — Final Design (logo only — large showcase) */}
            {logo && project.imagePath && (
              <motion.div variants={revealLightVariants} initial="hidden" whileInView="visible" viewport={viewport}>
                <SectionHeading number="04 —" title="Final Design" />
                <motion.div
                  whileHover={{ y: -4 }}
                  transition={{ duration: 0.25 }}
                  onClick={() => setViewerOpen(true)}
                  className="cursor-zoom-in overflow-hidden rounded-[1.8rem] border border-white/10 bg-gradient-to-br from-[#f8f6f2] to-[#ede9e0] p-10"
                >
                  <img
                    src={project.imagePath}
                    alt={`Bulaklaya Mjass logo designed for ${getClient(project) ?? "client"}`}
                    className="mx-auto max-h-[400px] w-auto object-contain drop-shadow-[0_12px_40px_rgba(0,0,0,0.20)]"
                  />
                </motion.div>
                <p className="mt-3 text-center text-xs text-slate-500">Click to view full size</p>
              </motion.div>
            )}

            {/* 04 / 05 — Design Process */}
            <motion.div variants={revealLightVariants} initial="hidden" whileInView="visible" viewport={viewport}>
              <SectionHeading number={logo ? "05 —" : "04 —"} title="Design Process" />
              <ProcessTimeline steps={project.process} />
            </motion.div>

            {/* 05 / 06 — My Role */}
            <motion.div variants={revealLightVariants} initial="hidden" whileInView="visible" viewport={viewport}>
              <SectionHeading number={logo ? "06 —" : "05 —"} title="My Role" />
              <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-6">
                <p className="mb-4 text-lg font-semibold text-white">{project.role.title}</p>
                <motion.ul variants={staggerContainer} initial="hidden" whileInView="visible" viewport={viewport} className="grid gap-2 sm:grid-cols-2">
                  {project.role.responsibilities.map((r) => (
                    <motion.li key={r} variants={staggerItem} className="flex items-center gap-2.5 text-sm text-slate-300">
            ...
            {/* 06 / 07 — Skills */}
            <motion.div variants={revealLightVariants} initial="hidden" whileInView="visible" viewport={viewport}>
              <SectionHeading number={logo ? "07 —" : "06 —"} title="Design Skills" />
              <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={viewport} className="flex flex-wrap gap-2.5">
            ...
            {/* 07 / 08 — Tools */}
            <motion.div variants={revealLightVariants} initial="hidden" whileInView="visible" viewport={viewport}>
              <SectionHeading number={logo ? "08 —" : "07 —"} title="Tools Used" />
            ...
            <motion.div
              ...
              whileInView="visible"
              viewport={viewport}
              className="rounded-[1.8rem] border border-orange-500/20 bg-orange-500/5 p-8 text-center"
            >
              <p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-orange-400">
                Work Together
              </p>
              ...
              <p className="mt-3 text-sm text-slate-400">
                Let&rsquo;s create something distinctive for your brand or business.
              </p>
              <motion.a
                href="#contact"
                onClick={onClose}
                ...
                whileTap={{ scale: 0.96 }}
                className="mt-6 inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-orange-500 to-orange-400 px-7 py-3 text-sm font-semibold text-white shadow-[0_0_20px_rgba(249,115,22,0.25)] transition-all duration-200"
              >
      ...
      <AnimatePresence>
        {viewerOpen && (
          <FullscreenViewer project={project} onClose={() => setViewerOpen(false)} />
        )}
      </AnimatePresence>
    </>
  );
